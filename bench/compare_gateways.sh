...
run_round() {
  ...
  .venv/bin/python bench/load_test.py --port 29101 --admin 29111 --subscribers ${=SUBS} \
    --seconds "$SECONDS_PER" --processes "$PROCS" --out "docs/data/load_gateway_$label.json"
  kill $GW; wait $GW 2>/dev/null
}
run_round python .venv/bin/python -m mdfeed.services.tcp_gateway
sleep 1
run_round cpp ./cpp/build/tcp_gateway
kill $FEEDD; wait $FEEDD 2>/dev/null
.venv/bin/python - <<'PY'
import json, datetime
py=json.load(open("docs/data/load_gateway_python.json")); cp=json.load(open("docs/data/load_gateway_cpp.json"))
rows=[]
...
out={"generated_at": datetime.datetime.now().astimezone().isoformat(timespec="seconds"),
     ...
     "rounds": rows}
json.dump(out, open("docs/data/gateway_compare.json","w"), ensure_ascii=False, indent=1)
