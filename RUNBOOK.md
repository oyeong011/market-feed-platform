...
새 바이너리로 바꾸면서 구독자 접속을 끊지 않는 절차다.

```bash
make cpp                      # 새 바이너리 빌드
ops/swap_gateway.sh           # 교체 + 교체 중 접속 실패 수 확인 (0 이 아니면 실패로 끝난다)
```

동작:
1. 새 프로세스가 `SO_REUSEPORT` 로 같은 포트에 함께 바인드한다. 커널이 새 접속을 나눠 준다.
2. 옛 프로세스에 `SIGTERM` → **리스너만 닫는다.** 이후 새 접속은 전부 새 프로세스로 간다.
3. 옛 프로세스는 기존 구독자에게 `MDFEED_DRAIN_SECONDS`(기본 5초) 동안 계속 배포하다 끝낸다.

확인할 것:
- 교체 중 `/readyz` 가 **503** 이면 그 프로세스는 드레인 중이다. 로드밸런서에서 빼라는 신호다.
- `/healthz` 의 `draining` 이 `true` 인 쪽이 옛 프로세스다. 겹치는 동안 관리 포트도 둘이 함께
  듣기 때문에 스크레이프가 어느 쪽에 닿을지 모른다 — 그 구간의 지표는 섞인다.
- 기존 구독자는 드레인이 끝나면 끊긴다. 재접속해서 **스냅샷으로 다시 맞춘다**. 조용히 빠지는
  것이 아니라 규약대로 재동기화하는 것이고, 클라이언트의 시퀀스 검사가 그걸 보증한다.
