...
- **장기 안정성은 시간 단위까지만 확인합니다.** 이제 매주 리눅스 러너에서 45분 관측이
  자동으로 돌고 결과가 산출물로 남습니다(`.github/workflows/soak.yml`). 하루·일주일 단위
  누수는 여전히 미확인이고, 그건 `make soak MINUTES=1440` 을 실제 서버에서 돌려야 압니다.
  하네스는 20분 미만 관측에서는 시간당 기울기로 판정하지 않습니다 — 짧은 표본을 외삽하면
  한 번의 흔들림이 거짓 양성이 됩니다.
