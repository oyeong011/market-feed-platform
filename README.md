...
>
> 위 표는 전부 macOS(kqueue) 수치입니다. 배포 대상인 리눅스(epoll)에서는 `.github/workflows/gateway-bench.yml` 이
> 매주 같은 비교를 돌려 산출물(`gateway_compare_linux.json`)로 남깁니다. 러너 성능이 흔들려 절대값이 아니라 상대 비교로 봅니다.
