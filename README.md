...
| 38 | **C++ 서비스가 자원 누수 감시 밖에 있었다** | 파이썬 서비스는 `/healthz` 의 `resources` 로 RSS·fd 를 내는데 C++ 게이트웨이와 멀티캐스트 발행자는 아무것도 안 냈다. 장시간 감시 하네스가 그 둘을 0MB/0fd 로 읽고 지나갔다 — **누수가 가장 보기 어려운 쪽(GC 가 없는 쪽)이 감시 밖**이었다 | 두 C++ 서비스에 같은 키 이름으로 자원 보고 추가(`mdfp/procstat.hpp`). 하네스는 `resources` 를 안 내는 서비스를 0 으로 세지 않고 건너뛴다. 주간 soak 이 시작 전에 "감시 대상이 실제로 자원을 보고하는지"부터 확인하고, 아니면 실패한다 |
| 39 | **첫 자동 soak 이 거짓 양성으로 실패** | 3개 서비스가 RSS 임계 초과로 걸렸는데 25분 창의 **실제 증가는 2.3~3.4MB** 였다. 시동 구간을 포함해 직선을 맞추고 시간당으로 외삽한 값이다. 하네스 문서가 같은 함정을 fd 에 대해 적어 두고 보호까지 넣어 놨으면서 RSS 에는 안 넣었다 | RSS 도 기울기와 절대 증가량(10MB)을 함께 본다. 워밍업(앞 5분)은 기울기에서 뺀다. **임계를 올려 초록으로 만들지 않았다** — 그건 숨기는 것이다 |
...
**메모리는 한 자릿수 MB입니다.** 리플레이 스택 실측에서 C++ 게이트웨이 1.4 MB, 멀티캐스트 발행자
3.1 MB 입니다. 같은 스택의 파이썬 서비스는 28~37 MB 입니다. 두 C++ 서비스도 파이썬과 같은 자리
(`/healthz` 의 `resources`)에 같은 이름으로 자원을 보고하므로 장시간 감시가 함께 봅니다.
...
- **장기 안정성은 시간 단위까지만 확인합니다.** 매주 리눅스 러너에서 150분 관측이 자동으로
  돌고 결과가 산출물로 남습니다(`.github/workflows/soak.yml`). 150분인 데는 이유가 있습니다 —
  RSS 임계 5 MB/h 짜리 누수가 절대 하한 10 MB 를 넘으려면 최소 2시간이 필요합니다. 그보다
  짧으면 **통과가 "누수 없음"이 아니라 "이 길이로는 모름"** 이고, 하네스가 결과에 그렇게 적습니다
  (`rss_verdict_reachable`). 하루·일주일 단위 누수는 여전히 미확인이며 실제 서버에서
  `make soak MINUTES=1440` 을 돌려야 압니다. 첫 리눅스 25분 실측(2026-09-25)에서는 8개 서비스
  전부 fd 증가 0 · 처리량 100% 이상 · RSS 절대 증가 3.4 MB 이하였습니다.
